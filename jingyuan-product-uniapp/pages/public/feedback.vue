      ...
      <view class="bg-white ss-p-x-20 ss-p-y-30 ss-m-t-20">
        ...
        <view class="textarea">
          <uni-easyinput
            :inputBorder="false"
            type="textarea"
            v-model="state.formData.mark"
            placeholderStyle="color:#BBBBBB;font-size:30rpx;font-weight:400;line-height:normal"
            placeholder="请描述您遇到的问题，建议上传照片"
            clearable
          ></uni-easyinput>
          <s-uploader
            v-model:url="state.formData.urls"
            fileMediatype="image"
            limit="9"
            mode="grid"
...
<script setup>
  ...
  async function onSubmit() {
    if (!state.formData.catalog) {
      sheep.$helper.toast('请选择类型');
      return;
    }
    if (!state.formData.mark) {
      sheep.$helper.toast('请描述您遇到的问题');
      return;
    }
    if (!state.formData.concat) {
  ...
  function radioChange(e) {
    state.formData.catalog = e.detail.value;
  }
</script>

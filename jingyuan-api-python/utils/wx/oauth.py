...
from apps.vadmin.auth.third.models import ThirdLoginVO
...
class WXOAuth:
    ...
    async def parsing(self, code: str) -> ThirdLoginVO:
        """
        解析openid
        :param code: 动态令牌。可通过动态令牌换取用户手机号。
        :return: openid | None
        ...
        rs = ThirdLoginVO(third_type="wechat", open_id=result["openid"], union_id=result['unionid'])
        return rs

...
class OpenAuth(AuthValidation):
    ...
    async def __call__(
        ...
        try:
            ...
            user = await UserDal(db).get_data(data_id=id, v_return_none=True)
...
class FullAdminAuth(AuthValidation):
    ...
    async def __call__(
        ...
        user = await UserDal(db).get_data(data_id=id, v_return_none=True, v_options=options, is_staff=True)

...
class UserDal(DalBase):
    ...
    async def update_current_avatar(self, user: models.VadminUser, file: UploadFile) -> str:
        ...
        :return:
        """
        result = await QiniuOSS(BucketConf(**settings.QINIU_OSS)).upload_image("avatar", file)
        user.avatar = result
        await self.flush(user)
        return result

    async def reset_current_mobile(self, user: models.VadminUser, data: schemas.ResetMobile, rd: Redis) -> None:
        """
        重置手机
        :param user:
        :param data:
        ...
        unique = await self.get_data(telephone=data.mobile, v_return_none=True)
        if unique:

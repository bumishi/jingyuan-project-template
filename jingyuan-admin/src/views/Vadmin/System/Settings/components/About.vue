...
const getData = async () => {
  ...
  if (res) {
    defaultHtml.value = res.data.web_about || ''
  }
}
...
const save = async () => {
  ...
  try {
    const res = ref({})
    res.value = await putSystemSettingsApi({ web_about: defaultHtml.value })
    if (res.value) {
      getData()
...
<template>
  ...
  <div class="mt-10px" style="float: right">
    <ElButton v-hasPermi="['system.setting.save']" :loading="loading" type="primary" @click="save">立即保存</ElButton>
  </div>
</template>
